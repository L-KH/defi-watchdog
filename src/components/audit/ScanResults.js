...
export default function ScanResults({ 
  ...
  // Combine all vulnerabilities from tools and AI
  const allVulnerabilities = [
    ...
    ...(aiResult?.risks?.map(risk => ({
      ...
      type: risk.title || 'ai_detection',
      line: 0,
      code: risk.codeReference || ''
    })) || [])
  ];
    ...
    const reportHTML = `
      ...
      <body>
        ...
        <div class="section">
          ...
          <p><strong>AI Model:</strong> DeepSeek Coder</p>
          <p><strong>Security Score:</strong> ${aiResult.securityScore}/100</p>
          <p><strong>Risk Level:</strong> ${aiResult.riskLevel}</p>
          ${aiResult.overview ? `<p><strong>Overview:</strong> ${aiResult.overview}</p>` : ''}
        </div>
        ` : ''}
              ...
              {aiResult && (
                <div className="border rounded-lg p-4">
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    <p><strong>Security Score:</strong> {aiResult.securityScore}/100</p>
                    <p><strong>Risk Level:</strong> {aiResult.riskLevel}</p>
                    <p><strong>Issues Found:</strong> {aiResult.risks?.length || 0}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="space-y-6">
            <div className="border-b pb-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">DeepSeek Coder</p>
                </div>
                <div>
                  <p className="text-gray-500">Security Score</p>
                  <p className="font-medium">{aiResult.securityScore}/100</p>
                </div>
                <div>
                  <p className="text-gray-500">Risk Level</p>
                  <p className="font-medium">{aiResult.riskLevel}</p>
                </div>
                <div>
                  <p className="text-gray-500">Issues Found</p>
                  <p className="font-medium">{aiResult.risks?.length || 0}</p>
                </div>
              </div>
            </div>
            ...
            {aiResult.overview && (
              <div className="p-4 bg-blue-50 rounded-lg">
                ...
                <p className="text-sm text-gray-700">{aiResult.overview}</p>
              </div>
            )}
            ...
            {aiResult.keyFeatures && aiResult.keyFeatures.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {aiResult.keyFeatures.map((feature, index) => (
                    <div key={index} className="p-2 bg-gray-50 rounded text-sm">
                      {feature}
                    </div>
                  ))}
